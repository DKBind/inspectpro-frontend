      ...
      {loading ? (
        ...
        <>
          <div className={styles.cardGrid}>
            {projects.map(p => {
              const statusMeta = STATUS_META[p.projectStatus as ProjectStatus] ?? { color: '#6B7280', bg: '#F3F4F6', label: p.projectStatus };
              return (
                <div key={p.id} className={styles.card}>
                  {/* Color strip */}
                  <div className={styles.cardStrip} style={{ background: statusMeta.color }} />
                  ...
                  <div className={styles.cardContent}>
                    ...
                    <div className={styles.cardInfo}>
                      ...
                      {p.totalBudget != null && (
                        <div className={styles.infoRow}>
                          <IndianRupee style={{ width: 13, height: 13, color: '#6B7280', flexShrink: 0 }} />
                          <span>₹ {Number(p.totalBudget).toLocaleString('en-IN')}</span>
                        </div>
                      )}
                    </div>
            ...
            <div className="px-7 py-6 space-y-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Fld label="Client" required error={errors.clientId?.message}>
                  ...
                  {clients.length === 0 && <p className="text-xs text-amber-500 mt-1">No clients yet. Add clients first.</p>}
                </Fld>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Fld label="City" hint="Optional"><Input placeholder="Mumbai" {...register('city')} className={inputCls(false)} /></Fld>
                <Fld label="State" hint="Optional"><Input placeholder="Maharashtra" {...register('state')} className={inputCls(false)} /></Fld>
              </div>

              <div className="grid gap-4 sm:grid-cols-2">
                <Fld label="Start Date" hint="Planned"><Input type="date" {...register('startDatePlanned')} className={inputCls(false)} /></Fld>
                <Fld label="End Date" hint="Estimated"><Input type="date" {...register('estimatedCompletionDate')} className={inputCls(false)} /></Fld>
              </div>

              <Fld label="Total Budget" hint="Optional (₹)">
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#9CA3AF] pointer-events-none"><IndianRupee size={14} /></span>
        ...
        <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto shadow-xl rounded-2xl p-0">
          ...
          {viewTarget && (
            <div className="px-7 py-5 space-y-0">
              ...
              <ViewRow label="Start Date"     value={fmt(viewTarget.startDatePlanned)} />
              ...
              <ViewRow label="Contract Value" value={viewTarget.contractValue != null ? `₹ ${Number(viewTarget.contractValue).toLocaleString('en-IN')}` : '—'} />
